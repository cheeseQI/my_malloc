...
void * bf_malloc(size_t size) {
    ...
    Node * curr = dummyHead;
    while (curr != NULL) {
        if (curr->allocated == 0 && curr->sz - INFO_SIZE == size) {
            blockBF = curr;
    ...
    if (blockBF->sz - INFO_SIZE > size && blockBF->sz  - INFO_SIZE <= size + INFO_SIZE) {
            blockBF->allocated = 1;
            return (void *) blockBF + INFO_SIZE;
    }
...
//gcc -c -Wall -Werror -fpic my_malloc.c
//gcc -shared -o libmymalloc.so my_malloc.o